...
import { getCachedTipperStats } from "@/lib/tipper-stats";

export default async function LeaderboardPage() {
  const { profile } = await requireUser();
  const stats = await getCachedTipperStats();
  const leaderboard = stats.profiles;
        ...
        <table className="min-w-[840px] w-full border-collapse text-left">
          <thead className="bg-ice-100 text-sm text-ice-900 dark:bg-slate-800 dark:text-slate-100">
            <tr>
              ...
              <th className="px-4 py-3">Trofeje</th>
          ...
          <tbody>
            {leaderboard.map((row) => (
              <tr key={row.user_id} className="border-t border-ice-100 dark:border-slate-700">
                ...
                <td className="px-4 py-4">
                  <TipperTrophyList trophies={row.trophies} />
                </td>
